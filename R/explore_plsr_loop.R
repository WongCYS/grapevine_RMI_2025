...
df_spectra <- read.csv(paste0(wd, "RMI_grapes/RMI2021/spectroscopy/processed_data/svc_clean_interp.csv"), check.names = F)
...
df_berry <- read.csv(paste0(wd, "RMI_grapes/RMI2021/berry_chemistry/berrychem_clean_CW.csv"))
...
#read in gasx data
df_gasx <- read.csv(paste0(wd, 'RMI_grapes/RMI2021/Physiology/Li-Cor Files/li6800_master.csv'))

#clean gasx df
df_gasx <- df_gasx %>%
  mutate(date = as.character(as.Date(date, format = "%Y%m%d %H:%M:%S"))) %>%
  rename(Row = row, Vine = vine, Block = block) %>%
  select(date, Block, Vine, Row, A, E, gsw, Tleaf) %>%
  group_by(date, Block, Vine, Row) %>%
  summarise_all(mean, na.rm = T)
...
df_data <- list(df_data, df_berry, df_gasx) %>%
...
#plot berry and gasx
plt_phys <- df_data %>%
  pivot_longer(cols = c(NDVI, PRI, TVI, brix, ph, ta_g_L), names_to = "param", values_to = "value") %>%
...
plt_ta <- df_data %>%
  ...
  ggplot(aes(date, ta_g_L, color=Variety, group = interaction(Block, Row, Vine)))+
  geom_point()+
  geom_line()
...
pls_rec_brix <- recipe(brix ~ ., data = perm_train) %>%
  update_role(NDVI, PRI, TVI, Variety, Block, Row, Vine, date, Region_of_Origin, variety, Variety_f, brix, ph, ta, ta_g_L, A, E, gsw, Tleaf, new_role = 'id') %>%
...
#recipe ta_g_L
pls_rec_ta <- recipe(ta_g_L ~ ., data = perm_train) %>%
  update_role(NDVI, PRI, TVI, Variety, Block, Row, Vine, date, Region_of_Origin, variety, Variety_f, brix, ph, ta, ta_g_L, A, E, gsw, Tleaf, new_role = 'id') %>%
  update_role(ta_g_L, new_role = 'outcome') %>%
  step_nzv(all_predictors()) %>%
  step_naomit(all_outcomes())

pls_rec_ta_veraison <- recipe(ta_g_L ~ ., data = perm_train_veraison) %>%
  update_role(NDVI, PRI, TVI, Variety, Block, Row, Vine, date, Region_of_Origin, variety, Variety_f, brix, ph, ta, ta_g_L, A, E, gsw, Tleaf, new_role = 'id') %>%
  update_role(ta_g_L, new_role = 'outcome') %>%
  step_nzv(all_predictors()) %>%
  step_naomit(all_outcomes())
...
rec_list <- list(pls_rec_brix, pls_rec_ta, pls_rec_brix_veraison, pls_rec_ta_veraison)
pred_list <- c("brix", "ta_g_L", "brix_veraison", "ta_g_L_veraison")
df_tune <- data.frame()
df_vip <- data.frame()
df_train <- data.frame()
df_test <- data.frame()
...
plt_vip <- df_vip %>% 
  ...
  scale_color_manual(NULL,values = c("#E07A5F", "#3D405B", "#81B29A", "#F2CC8F"), labels = c("Tartaric Acid - Post-Veraison", "Tartaric Acid - Full Period", "Brix - Post-Veraison", "Brix - Full Period"))+
  theme_bw()
...
col_all <- c('#e6c229', '#f17105', '#d11149', '#6610f2')
col_ver <- c('#f17105', '#d11149', '#6610f2')
...
plt_sp_ta_plsr <- df_test %>% 
  ...
  geom_smooth(aes(color = NULL), color = 'black', method = 'lm', formula = y~x, se = F) +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed')+
  scale_x_continuous(expression(atop("Predicted", "Tartaric acid (g/L)")), limits = c(0,50))+
  scale_y_continuous(expression(atop("Observed", "Tartaric acid (g/L)")), limits = c(0,50))+
  scale_color_manual(values = col_all)+
  annotate("text", x = 0, y = Inf, vjust = 1.35, hjust = 0, 
           label = paste("RMSPE =", round(df_stats[df_stats$param == "ta_g_L", "rmspe"]$rmspe, 2), "%",
                         "\nR? =", round(df_stats[df_stats$param == "ta_g_L", "rsq"]$rsq, 2),
                         cut(df_stats[df_stats$param == "ta_g_L", "p"]$p, 
                             breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                             labels = c("***", "**", "*", ""))))+
  theme_bw()+theme(aspect.ratio = 1, legend.title = element_blank())
...
plt_sp_ta_plsr_v <- df_test %>% 
  ...
  scale_color_manual(values = col_ver)+
  annotate("text", x = 0, y = Inf, vjust = 1.35, hjust = 0, 
           label = paste("RMSPE =", round(df_stats[df_stats$param == "ta_g_L_veraison", "rmspe"]$rmspe, 2), "%",
                         "\nR? =", round(df_stats[df_stats$param == "ta_g_L_veraison", "rsq"]$rsq, 2),
                         cut(df_stats[df_stats$param == "ta_g_L_veraison", "p"]$p, 
                             breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                             labels = c("***", "**", "*", ""))))+
  theme_bw()+theme(aspect.ratio = 1, legend.title = element_blank())
...
plt_sp_ta_ndvi <- df_test %>% 
  ...
  scale_color_manual(values = col_all)+
  annotate("text", x = .5, y = Inf, vjust = 2, hjust = 0, 
           label = paste("R? =", round(df_stats_ndvi[df_stats_ndvi$param == "ta_g_L", "rsq"]$rsq, 2),
                         cut(df_stats_ndvi[df_stats_ndvi$param == "ta_g_L", "p"]$p, 
                             breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                             labels = c("***", "**", "*", ""))))+
  theme_bw()+theme(aspect.ratio = 1, legend.title = element_blank())
...
plt_sp_ta_ndvi_v <- df_test %>% 
  ...
  scale_color_manual(values = col_ver)+
  annotate("text", x = .5, y = Inf, vjust = 2, hjust = 0, 
           label = paste("R? =", round(df_stats_ndvi[df_stats_ndvi$param == "ta_g_L_veraison", "rsq"]$rsq, 2),
                         cut(df_stats_ndvi[df_stats_ndvi$param == "ta_g_L_veraison", "p"]$p, 
                             breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                             labels = c("***", "**", "*", ""))))+
  theme_bw()+theme(aspect.ratio = 1, legend.title = element_blank())
...
plt_sp_ta_pri <- df_test %>% 
  ...
  scale_color_manual(values = col_all)+
  annotate("text", x = 0, y = Inf, vjust = 2, hjust = 0, 
           label = paste("R? =", round(df_stats_pri[df_stats_pri$param == "ta_g_L", "rsq"]$rsq, 2),
                         cut(df_stats_pri[df_stats_pri$param == "ta_g_L", "p"]$p, 
                             breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                             labels = c("***", "**", "*", ""))))+
  theme_bw()+theme(aspect.ratio = 1, legend.title = element_blank())
...
plt_sp_ta_pri_v <- df_test %>% 
  ...
  scale_color_manual(values = col_ver)+
  annotate("text", x = 0, y = Inf, vjust = 2, hjust = 0, 
           label = paste("R? =", round(df_stats_pri[df_stats_pri$param == "ta_g_L_veraison", "rsq"]$rsq, 2),
                         cut(df_stats_pri[df_stats_pri$param == "ta_g_L_veraison", "p"]$p, 
                             breaks = c(-Inf, 0.001, 0.01, 0.05, Inf),
                             labels = c("***", "**", "*", ""))))+
  theme_bw()+theme(aspect.ratio = 1, legend.title = element_blank())
...
ggarrange(NULL, as_ggplot(text_grob('NDVI', vjust = 3, hjust = 3.2, size = 14)),
          NULL, as_ggplot(text_grob('PLSR', vjust = 3, hjust = 3, size = 14)),
          as_ggplot(text_grob('Full period', vjust = 3, hjust = .8, size = 14)), NULL, NULL, NULL,
          plt_sp_brix_ndvi, plt_sp_ta_ndvi, plt_sp_brix_plsr, plt_sp_ta_plsr,
          as_ggplot(text_grob('Post-Veraison', vjust = 3, hjust = .6, size = 14)), NULL, NULL, NULL,
          plt_sp_brix_ndvi_v, plt_sp_ta_ndvi_v, plt_sp_brix_plsr_v, plt_sp_ta_plsr_v,
          ncol = 4, nrow = 5, common.legend = T, legend = "bottom", align = "hv", heights = c(.1,.1,1,.1,1,1),
...
#brix
plt_ts_brix <- df_data %>%
  select(date, Variety_f, brix, ta_g_L, NDVI) %>%
  ...
  scale_x_date(NULL, limits = c(as.Date("2021-06-15"), as.Date("2021-10-01")), date_labels = "%b")+
...
plt_ts_ta <- df_data %>%
  ...
  group_by(date, Variety_f) %>%
  summarise_all(mean, na.rm = T) %>%
  ggplot(aes(date, ta_g_L, color = Variety_f))+
  geom_line(show.legend = F)+
  geom_text(data = . %>%
              group_by(Variety_f) %>%
              drop_na(Variety_f) %>%
